...
def test_dependency_rows_cascade_on_owner_delete(db_session):
    async def _run():
        override = app.dependency_overrides[get_db]
        async for session in override():
            product = Product(name="Handoff", key_prefix="HAND")
            ...
            await session.delete(task_a)
            await session.delete(feature_a)
            await session.commit()

            result = await session.execute(
                select(TaskDependency).where(TaskDependency.depends_on_task_id == task_b.id)
            )
            assert result.scalar_one_or_none() is None

            result = await session.execute(
                select(FeatureDependency).where(
                    FeatureDependency.depends_on_feature_id == feature_b.id
                )
            )
            assert result.scalar_one_or_none() is None
            break
